...
import static com.yjl.fastjson.parser.JSONLexer.EOI;
import static com.yjl.fastjson.parser.JSONToken.EOF;
import static com.yjl.fastjson.parser.JSONToken.ERROR;
import static com.yjl.fastjson.parser.JSONToken.FALSE;
import static com.yjl.fastjson.parser.JSONToken.HEX;
import static com.yjl.fastjson.parser.JSONToken.IDENTIFIER;
import static com.yjl.fastjson.parser.JSONToken.LBRACE;
import static com.yjl.fastjson.parser.JSONToken.LBRACKET;
import static com.yjl.fastjson.parser.JSONToken.LITERAL_FLOAT;
import static com.yjl.fastjson.parser.JSONToken.LITERAL_INT;
import static com.yjl.fastjson.parser.JSONToken.LITERAL_STRING;
import static com.yjl.fastjson.parser.JSONToken.NEW;
import static com.yjl.fastjson.parser.JSONToken.NULL;
import static com.yjl.fastjson.parser.JSONToken.RBRACKET;
import static com.yjl.fastjson.parser.JSONToken.SET;
import static com.yjl.fastjson.parser.JSONToken.TREE_SET;
import static com.yjl.fastjson.parser.JSONToken.TRUE;
import static com.yjl.fastjson.parser.JSONToken.UNDEFINED;
...
import com.yjl.fastjson.parser.deserializer.ResolveFieldDeserializer;
...
public class DefaultJSONParser implements Closeable {
    ...
    static {
        Class<?>[] classes = new Class[] {boolean.class, byte.class, short.class, int.class,
                long.class, float.class, double.class,

                Boolean.class, Byte.class, Short.class, Integer.class, Long.class, Float.class,
                Double.class,
    ...
    public DefaultJSONParser(final char[] input, int length, final ParserConfig config,
            int features) {
            ...
            for (;;) {
                            ...
                            if (deserializer instanceof JavaBeanDeserializer) {
                                ...
                                for (Object o : map.entrySet()) {
                                    ...
                                    if (entryKey instanceof String) {
                                        FieldDeserializer fieldDeserializer = javaBeanDeserializer
                                                .getFieldDeserializer((String) entryKey);
                ...
                if (ch == '"') {
                    ...
                    if (lexer.token() == JSONToken.RBRACE) {
                        ...
                        throw new JSONException(
                                "syntax error, position at " + lexer.pos() + ", name " + key);
                    }
                }
    ...
    public void parseArray(Type type, Collection array, Object fieldName) {
        ...
        if (token != JSONToken.LBRACKET) {
            throw new JSONException(
                    "exepct '[', but " + JSONToken.name(token) + ", " + lexer.info());
        }
        ...
        for (;;) {
            ...
            if (fieldDeser == null) {
                ...
                if (fieldClass == int.class) {
                    ...
                    ObjectDeserializer fieldValueDeserializer =
                            config.getDeserializer(fieldClass, fieldType);
    ...
    public void checkMapResolve(Map object, Object fieldName) {
        if (resolveStatus == NeedToResolve) {
            ResolveFieldDeserializer fieldResolver =
                    new ResolveFieldDeserializer(object, fieldName);
            ResolveTask task = getLastResolveTask();
    ...
    public final void parseArray(final Collection array, Object fieldName) {
        ...
        if (lexer.token() != JSONToken.LBRACKET) {
            throw new JSONException(
                    "syntax error, expect [, actual " + JSONToken.name(lexer.token()) + ", pos "
        ...
        try {
            if (lexer.isEnabled(Feature.AutoCloseSource)) {
                if (lexer.token() != JSONToken.EOF) {
                    throw new JSONException(
                            "not close json text, token : " + JSONToken.name(lexer.token()));
                }
            }
        } finally {
            lexer.close();
