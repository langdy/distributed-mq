...
public class FieldInfo implements Comparable<FieldInfo> {
    ...
    public static Type getFieldType(final Class<?> clazz, final Type type, Type fieldType) {
        ...
        if (fieldType instanceof GenericArrayType) {
            ...
            if (componentType != componentTypeX) {
                Type fieldTypeX =
                        Array.newInstance(TypeUtils.getClass(componentTypeX), 0).getClass();
                return fieldTypeX;
            }
        ...
        if (fieldType instanceof ParameterizedType) {
            ...
            boolean changed =
                    getArgument(arguments, typeVariables, paramType.getActualTypeArguments());
            if (changed) {
                fieldType = new ParameterizedTypeImpl(arguments,
                        parameterizedFieldType.getOwnerType(), parameterizedFieldType.getRawType());
                return fieldType;
            }
        }
    ...
    private static boolean getArgument(Type[] typeArgs, TypeVariable[] typeVariables,
            Type[] arguments) {
            ...
            if (typeArg instanceof ParameterizedType) {
                ...
                if (p_changed) {
                    typeArgs[i] = new ParameterizedTypeImpl(p_typeArg_args,
                            p_typeArg.getOwnerType(), p_typeArg.getRawType());
                    changed = true;
                }
    ...
    private static Type getInheritGenericType(Class<?> clazz, Type type, TypeVariable<?> tv) {
        ...
        if (class_gd == clazz) {
            ...
            for (Class<?> c = clazz; c != null && c != Object.class && c != class_gd; c =
                    c.getSuperclass()) {
